...
def frame_topic(trend: str) -> str:
    ...
    user = (
        ...
        "『資訊型/科普型』YouTube 影片的具體主題（繁體中文，一句話）。\n"
        "目標是『衝 YouTube 流量與訂閱』，所以要挑『有高搜尋需求、廣泛受眾、能引發好奇而想點進來看』"
        "的角度（避免太冷門小眾）。\n"
        "嚴格避開：政治/政黨/選舉/政治人物、色情/性、暴力/犯罪、醜聞/八卦/個人隱私、宗教爭議。\n"
        "若這個關鍵字屬於上述敏感類別，請『完全改成』一個中性、正面、有知識含量又熱門的主題"
        "（可以與關鍵字無關，例如科學、健康、科技、理財、生活、歷史文化、自然）。"
    )
    for _ in range(3):
        try:
